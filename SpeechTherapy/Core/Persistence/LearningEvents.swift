...
struct LearningEvent: Identifiable, Equatable {
    ...
    init(
        ...
        occurredAt: Date = .now,
        sessionID: UUID,
        activityID: String = "hidden-objects",
        sceneID: String,
        targetID: String? = nil,
        eventType: LearningEventType,
        attemptNumber: Int? = nil,
        outcome: LearningOutcome? = nil,
        activeDurationMs: Int? = nil,
...
final class LearningEventRecord {
    ...
    var event: LearningEvent? {
        guard let type = LearningEventType(rawValue: eventTypeRaw) else { return nil }
        return LearningEvent(
            ...
            outcome: outcomeRaw.flatMap(LearningOutcome.init(rawValue:)),
...
final class SwiftDataProgressRepository: ProgressRepository {
    ...
    func allEvents() throws -> [LearningEvent] {
        let descriptor = FetchDescriptor<LearningEventRecord>(sortBy: [SortDescriptor(\\.occurredAt)])
        return try context.fetch(descriptor).compactMap(\\.event)
    }

    func resetAll() throws {
        let descriptor = FetchDescriptor<LearningEventRecord>()
